...
from .Elements import SynchronousGenerator, Line, Switch, TwoWindingTransformer, ThreeWindingTransformer, Load, Busbar, ExternalGrid, VoltageSourceAC, CommonImpedance, ShuntElement
import cmath

class Converter:
    @staticmethod
    def convert_busbars(pf_busbars: List[DataObject]) -> List[Busbar]:
        busbars: List[Busbar] = []
        for id, busbar in enumerate(pf_busbars):
            new_busbar = Busbar(
                name=busbar.GetAttribute('loc_name'),
                id=id,
                substation=busbar.GetAttribute('cpSubstat'),
            )
            busbars.append(new_busbar)
        return busbars
    
    @staticmethod
    def convert_lines(pf_lines: List[DataObject], busbar_name_to_index: object) -> List[Line]:
        """
        ...
        for line in pf_lines:
            # Get line type data
            line_type: DataObject = line.GetAttribute('typ_id')
            if not line_type:
    ...
    @staticmethod
    def convert_synchronous_machines(machines: List[DataObject]) -> List[SynchronousGenerator]:
        generators: List[SynchronousGenerator] = []
        for machine in machines:
            # Get machine data
            machine_type: DataObject = machine.GetAttribute("typ_id")
    ...
    @staticmethod
    def convert_switches(switches: List[DataObject]) -> List[Switch]:
        switchs: List[Switch] = []
        for switch in switches:
            ...
            element_id = Converter.generate_unique_busbar_name(switch.GetAttribute("loc_name"), substation_name)

            # Get switch type data if there is any otherwise set default value
            try:
                swith_type: DataObject = switch.GetAttribute('typ_id')
    ...
    @staticmethod
    def convert_two_winding_transformers(pf_transformers: List[DataObject]) -> List[TwoWindingTransformer]:
        ...
        for transformer in pf_transformers:
            ...
            trafo_type: DataObject = transformer.GetAttribute("typ_id")
    ...
    @staticmethod
    def convert_three_winding_transformers(pf_transformers: List[DataObject]) -> List[ThreeWindingTransformer]:
    ...
    @staticmethod
    def convert_loads(pf_loads: List[DataObject], load_flow_results: Dict[str, BusLoadFlowResult]) -> List[Load]:
        loads: List[Load] = []
    ...
    @staticmethod
    def convert_shunt_elements(pf_shunt_elements: List[DataObject]) -> List[ShuntElement]:
        ...
        for shunt_element in pf_shunt_elements:
            ...
            connected_terminal: DataObject = shunt_element.GetAttribute("bus1")
            ...
            new_shunt_element = ShuntElement(
                ...
                ShuntElementPF=shunt_element
            )
            shunt_elements.append(new_shunt_element)
        return shunt_elements
    
    @staticmethod
    def convert_external_grids(pf_external_grids: List[DataObject]) -> List[ExternalGrid]:
    ...
    @staticmethod
    def convert_voltage_source_AC(pf_voltage_sources_AC: List[DataObject]) -> List[VoltageSourceAC]:
    ...
    @staticmethod
    def convert_common_impedances(pf_common_impedances: List[DataObject]) -> List[CommonImpedance]:
    ...
    @staticmethod
    def generate_unique_busbar_name(busbar_name: str, substation_name: str) -> str:
        unique_key = f"{substation_name}_{busbar_name}"
        # unique_key = f"{busbar_name}"
        return unique_key